...
def change_instruction(mode_checkbox_group: str):
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            mode_checkbox_group = gr.Radio(
                choices=inference_mode_list, label="选择推理模式", value=inference_mode_list[0]
            )
            instruction_text = gr.Text(
                label="操作步骤", value=instruct_dict[inference_mode_list[0]], scale=2
            )
            sft_dropdown = gr.Dropdown(
                choices=sft_spk, label="选择预训练音色", value=sft_spk[0], scale=1
            )
            stream = gr.Radio(
                choices=stream_mode_list, label="是否流式推理", value=stream_mode_list[0][1]
            )
            speed = gr.Number(
                value=1, label="速度调节(仅支持非流式推理)", minimum=0.5, maximum=2.0, step=0.1
            )
        ...
        with gr.Row():
            prompt_wav_upload = gr.Audio(
                sources="upload", type="filepath", label="选择prompt音频文件，注意采样率不低于16khz"
            )
        ...
        instruct_text = gr.Textbox(
            label="输入instruct文本", lines=1, placeholder="请输入instruct文本.", value=""
        )
        ...
        mode_checkbox_group.change(
            fn=change_instruction, inputs=[mode_checkbox_group], outputs=[instruction_text]
        )

...
class CosyVoice:
    ...
    def add_zero_shot_spk(self, prompt_text, prompt_speech_16k, zero_shot_spk_id) -> bool:
    ...
    def inference_sft(
        self, tts_text: str, spk_id: str, stream=False, speed=1.0, text_frontend=True
    ):
        """
        预训练音色
        """
    ...
    def inference_cross_lingual(
        ...
        with tqdm(
            ...
            for i in i_bar:
                ...
                logging.debug(f"synthesis text {i}")
                start_time = time.time()
                for model_output in self.model.tts(**model_input, stream=stream, speed=speed):
                    speech_len = model_output["tts_speech"].shape[1] / self.sample_rate
...
class CosyVoice2(CosyVoice):
    def __init__(
        ...
        if use_torchair:
            ...
            logging.info("Success对hift模型结构进行torchair图模式适配")
        else:
            logging.warning("Failed 对hift模型结构进行torchair图模式适配")

        del configs

    def inference_instruct(self, *args, **kwargs) -> NotImplementedError:
    ...
    def inference_instruct2(
        ...
        for i in tqdm(
            ...
            for model_output in self.model.tts(**model_input, stream=stream, speed=speed):
                speech_len = model_output["tts_speech"].shape[1] / self.sample_rate
                logging.info(
                    f"yield speech len {speech_len}, rtf {(time.time() - start_time) / speech_len:3f}"
                )
                yield model_output
                start_time = time.time()

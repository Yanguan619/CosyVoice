...
class CosyVoiceModel:
    ...
    def load(self, llm_model, flow_model, hift_model):
        self.llm.load_state_dict(
            torch.load(llm_model, map_location=self.device, weights_only=False), strict=True
        )
        self.llm.to(self.device).eval()
        self.flow.load_state_dict(
            torch.load(flow_model, map_location=self.device, weights_only=False), strict=True
        )
        self.flow.to(self.device).eval()
        # in case hift_model is a hifigan model
        hift_state_dict = {
            k.replace("generator.", ""): v
            for k, v in torch.load(hift_model, map_location=self.device, weights_only=False).items()
        }
        self.hift.load_state_dict(hift_state_dict, strict=True)
        self.hift.to(self.device).eval()

    def load_jit(self, llm_text_encoder_model, llm_llm_model, flow_encoder_model) -> None:
        self.llm.text_encoder = torch.jit.load(llm_text_encoder_model, map_location=self.device)
        self.llm.llm = torch.jit.load(llm_llm_model, map_location=self.device)
    ...
    def load_trt(self, flow_decoder_estimator_model, flow_decoder_onnx_model, trt_concurrent, fp16):
        ...
        if (
            ...
            convert_onnx_to_trt(
                flow_decoder_estimator_model, self.get_trt_kwargs(), flow_decoder_onnx_model, fp16
            )
        del self.flow.decoder.estimator
        import tensorrt as trt

        with open(flow_decoder_estimator_model, "rb") as f:
            estimator_engine = trt.Runtime(trt.Logger(trt.Logger.INFO)).deserialize_cuda_engine(
                f.read()
            )
    ...
    def llm_job(self, text, prompt_text, llm_prompt_speech_token, llm_embedding, uuid):
        with (
            self.llm_context(),
            torch.amp.autocast(
                str(self.device), enabled=self.fp16 is True and hasattr(self.llm, "vllm") is False
            ),
        ):
    ...
    def token2wav(
        ...
        with torch.amp.autocast(str(self.device), enabled=self.fp16):
        ...
        if stream is True:
            ...
            while True:
                ...
                if (
                    ...
                    with self.lock:
                        self.tts_speech_token_dict[this_uuid] = self.tts_speech_token_dict[
                            this_uuid
                        ][token_hop_len:]
                    # increase token_hop_len for better speech quality
                    token_hop_len = min(
                        self.token_max_hop_len, int(token_hop_len * self.stream_scale_factor)
                    )
                if (
            ...
            this_tts_speech_token = torch.tensor(self.tts_speech_token_dict[this_uuid]).unsqueeze(
                dim=0
            )
            this_tts_speech = self.token2wav(
                token=this_tts_speech_token,
...
class CosyVoice2Model(CosyVoiceModel):
    def __init__(
        self, llm: torch.nn.Module, flow: torch.nn.Module, hift: torch.nn.Module, fp16: bool = False
    ):
        self.no_use_cpu = True
        self.device = torch.device(
            DEVICE if torch.cuda.is_available() and self.no_use_cpu else "cpu"
        )
        self.llm = llm
        self.flow = flow
        self.hift = hift
    ...
    def load_jit(self, flow_encoder_model: str | Path) -> None:
        self.flow.encoder = torch.jit.load(flow_encoder_model, map_location=self.device)
    ...
    def load_vllm(self, model_dir):
        ...
        self.llm.vllm = LLMEngine.from_engine_args(
            EngineArgs(
                model=model_dir,
    ...
    def tts(
        ...
        **kwargs,
        ...
        if stream is True:
            ...
            this_token_hop_len = (
                self.token_hop_len + prompt_token_pad if token_offset == 0 else self.token_hop_len
            )
            if (
